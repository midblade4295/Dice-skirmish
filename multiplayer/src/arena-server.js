...
class Matchmaker{
 constructor({store=new Store(null),now=()=>Date.now(),seed=()=>crypto.randomBytes(4).readUInt32LE(),searchMs=A.SEARCH_MS}={}){
  ...
  for(const [rid,r]of Object.entries(store.data.rooms)){const e=A.Engine.restore(r.engine);e.random=serverRandom;this.rooms.set(rid,{engine:e,acks:r.acks||{},completed:r.completed||false});for(const h of e.s.heroes)if(!h.bot&&!store.data.users[h.id]?.receipts[rid]?.claimed)this.membership.set(h.id,{type:'room',id:rid});}
 }
 createSession(name){const tid=token(),pid=id(),now=this.now();const u={id:pid,tokenHash:sha(tid),name:clean(name)||'Player '+pid.slice(0,4),createdAt:now,lastSeen:now,mastery:{},receipts:{},guildId:null,days:{},earned:{gold:0,fate:0,pts:0,shards:0,xp:0,tokens:0}};this.store.data.users[pid]=u;this.store.save();return {playerId:pid,token:tid,name:u.name,protocol:A.VERSION};}
 auth(raw){assert(raw&&raw.length<300,'Sign in to the arena',401);const hash=sha(raw),u=Object.values(this.store.data.users).find(x=>x.tokenHash===hash);assert(u,'Arena session expired',401);u.lastSeen=this.now();return u;}
 ...
 join(u,prefs){const old=this.membership.get(u.id);if(old)return this.state(u);const load=A.loadout(prefs.loadout);assert(Number.isInteger(prefs.char)&&prefs.char>=0&&prefs.char<5,'Invalid hero');assert(Number.isInteger(prefs.weapon)&&prefs.weapon>=0&&prefs.weapon<9,'Invalid weapon');const now=this.now();
 ...
 tick(){const now=this.now();for(const l of [...this.lobbies]){const gone=l.players.filter(p=>now-p.lastSeen>8000);for(const p of gone)this.membership.delete(p.id);l.players=l.players.filter(p=>now-p.lastSeen<=8000);if(!l.players.length){this.lobbies=this.lobbies.filter(x=>x!==l);continue;}if(now<l.deadline)continue;
   ...
   const engine=new A.Engine({id:l.id,players,now,seed:this.seed()});engine.random=serverRandom;this.rooms.set(l.id,{engine,acks:{},completed:false});for(const p of l.players)this.membership.set(p.id,{type:'room',id:l.id});this.lobbies=this.lobbies.filter(x=>x!==l);this.dirty=true;
  }
  for(const [rid,r]of this.rooms){const s=r.engine.s;if(!s.ended){for(const h of s.heroes)if(!h.bot){h.connected=now-h.lastSeen<=8000;h.substitute=!h.connected;}r.engine.tick(now);this.dirty=true;}
   if(s.ended&&!r.completed){r.completed=true;for(const h of s.heroes.filter(h=>!h.bot))this.awardResult(this.user(h.id),r.engine.result(h.id));this.dirty=true;}
   if(s.ended&&now-s.completedAt>86400000){for(const h of s.heroes)if(this.membership.get(h.id)?.id===rid)this.membership.delete(h.id);this.rooms.delete(rid);delete this.store.data.rooms[rid];}
  }
 }
 state(u){const m=this.membership.get(u.id),now=this.now();if(!m)return {status:'idle',serverNow:now};if(m.type==='queue'){const l=this.lobbies.find(x=>x.id===m.id);if(!l){this.membership.delete(u.id);return {status:'idle',serverNow:now};}const p=l.players.find(x=>x.id===u.id);if(p)p.lastSeen=now;return {status:'searching',lobbyId:l.id,deadline:l.deadline,serverNow:now,humans:l.players.length,capacity:A.CAPACITY,waitMs:this.searchMs};}
  const r=this.rooms.get(m.id);assert(r,'Match unavailable',404);const h=r.engine.hero(u.id);h.lastSeen=now;h.connected=true;h.substitute=false;return {status:r.engine.s.ended?'complete':'battle',serverNow:now,match:r.engine.snapshot(),result:r.engine.s.ended?r.engine.result(u.id):null,receipt:r.engine.s.ended?u.receipts[m.id]||null:null};
 }
 act(u,request){const m=this.membership.get(u.id);assert(m?.type==='room','No active battle',409);const r=this.rooms.get(m.id);assert(request.matchId===m.id,'Stale match action',409);assert(typeof request.actionId==='string'&&/^[\w-]{8,80}$/.test(request.actionId),'Invalid action identifier');const key=u.id+':'+request.actionId;if(r.acks[key])return {...r.acks[key],state:this.state(u),replayed:true};
  assert(!r.engine.hero(u.id).substitute,'Reconnect before acting',409);const out=r.engine.act(u.id,request,this.now());r.acks[key]={result:out};/* Keep action receipts for the whole match; replaying an old ID must never act twice. */this.dirty=true;return {result:out,state:this.state(u)};
 }
 awardResult(u,r){if(u.receipts[r.id])return u.receipts[r.id];const d=u.days[day(this.now())]||={fate:0,expedition:0};const reward={...r.reward,fate:Math.min(r.reward.fate,Math.max(0,20-d.fate))};d.fate+=reward.fate;
  ...
  const receipt={id:r.id,at:this.now(),char:r.char,weapon:r.weapon,eligible:r.eligible,reward,mastery:r.eligible?r.mastery:{assault:0,guardian:0,commander:0},expedition,claimed:false};u.receipts[r.id]=receipt;return receipt;
 }
...
function createServer({dataFile=null,allowedOrigins=[],allowLocal=false,trustProxy=false,now,rateLimit=120,staticFile=null}={}){
 ...
 const server=http.createServer(async(req,res)=>{const origin=req.headers.origin,allowed=!origin||allowedOrigins.includes(origin)||(allowLocal&&origin==='null');
  const json=(status,obj)=>{let b=Buffer.from(JSON.stringify(obj));res.setHeader('Content-Type','application/json');res.setHeader('Cache-Control','no-store');res.setHeader('X-Content-Type-Options','nosniff');if(origin&&allowed){res.setHeader('Access-Control-Allow-Origin',origin);res.setHeader('Vary','Origin');}if(b.length>1000&&/gzip/.test(req.headers['accept-encoding']||'')){b=zlib.gzipSync(b);res.setHeader('Content-Encoding','gzip');}res.writeHead(status);res.end(b);};
  try{assert(allowed,'Origin is not allowed',403);const ip=trustProxy?String(req.headers['x-forwarded-for']||req.socket.remoteAddress).split(',')[0].trim():req.socket.remoteAddress;
   ...
   const url=new URL(req.url,'http://localhost'),route=url.pathname.replace(/^\/fatebound\/arena/,'').replace(/^\/arena/,'');
   ...
   if(route==='/health'){json(200,{ok:true,game:'Fatebound',protocol:1,queueMs:A.SEARCH_MS,capacity:A.CAPACITY,rooms:mm.rooms.size});return;}
   if(route==='/'&&staticFile){res.setHeader('Content-Type','text/html; charset=utf-8');fs.createReadStream(staticFile).pipe(res);return;}
   let body={};if(req.method==='POST'){let size=0,chunks=[];for await(const chunk of req){size+=chunk.length;assert(size<=4096,'Request too large',413);chunks.push(chunk);}try{body=JSON.parse(Buffer.concat(chunks).toString()||'{}');}catch(_){throw Error('Invalid JSON');}}
   if(route==='/session'&&req.method==='POST'){const sk=ip+':session:'+bucket,c=(rates.get(sk)||0)+1;rates.set(sk,c);assert(c<=10,'Too many guest sessions',429);json(201,mm.createSession(body.name));return;}
   ...
   if(route==='/state')mm.state(u);mm.tick();let out;
   ...
   if(req.method==='POST')mm.flush();json(200,out);
  }catch(e){json(e.status||400,{error:clean(e.message,160)});}
 });
