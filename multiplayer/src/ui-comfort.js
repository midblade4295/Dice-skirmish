...
function openSound(){audioOpener=document.activeElement;const box=modal('soundSettings','Sound studio');const p=SFX.volumes;
 box.querySelector('.comfort-scroll').innerHTML=`<p class="comfort-note">Original weapon, impact, magic and menu sounds. Combat and interface levels can be adjusted separately.</p>${[['master','Master volume'],['combat','Combat & spells'],['ui','Menus & rewards']].map(([key,label])=>`<label class="audio-level" for="audio-${key}"><span>${label}<output id="audioValue-${key}">${Math.round(p[key]*100)}%</output></span><input id="audio-${key}" data-audio-level="${key}" type="range" min="0" max="100" step="1" value="${Math.round(p[key]*100)}"></label>`).join('')}<button id="audioMute"></button><h3>Try the sounds</h3><div class="audio-previews"><button data-preview="sword">Sword</button><button data-preview="crit">Critical hit</button><button data-preview="bulwark">Bulwark</button><button data-preview="horn">War Horn</button><button data-preview="surge">Arcane Surge</button><button data-preview="menuOpen">Menu</button></div><p class="comfort-note">Sound respects your mute setting and stops when the game is in the background.</p>`;
 const paint=()=>{const b=$('audioMute');b.textContent=SFX.muted?'Unmute all sounds':'Mute all sounds';b.setAttribute('aria-pressed',String(SFX.muted));};paint();
 box.querySelectorAll('[data-audio-level]').forEach(input=>{input.oninput=()=>{const key=input.dataset.audioLevel;SFX.setVolumes({[key]:Number(input.value)/100});$('audioValue-'+key).textContent=input.value+'%';};});
 $('audioMute').onclick=()=>{SFX.toggle();paint();document.querySelectorAll('.mute').forEach(b=>b.textContent=SFX.muted?'🔇':'🔊');};
 box.querySelectorAll('[data-preview]').forEach(b=>b.onclick=()=>{SFX.unlock();SFX.play(b.dataset.preview);});
 box.querySelector('footer').innerHTML='<button id="soundDone">Done</button>';$('soundDone').onclick=()=>close('soundSettings');box.hidden=false;SFX.menuOpen();box.querySelector('.comfort-close').focus({preventScroll:true});
}
function install(){
