...
const SPELLS={barrage:{name:'Barrage',desc:'A free 1.5× sword attack.'},bulwark:{name:'Bulwark',desc:'8 seconds of half damage and a shield for allies here.'},horn:{name:'War Horn',desc:'Rally teammates here without spending Focus.'},surge:{name:'Arcane Surge',desc:'+4 Focus and +25% roll results for 10 seconds.'}};
...
class Engine{
 constructor({id,players,now,seed=12345,mode='standard',duration=MATCH_MS,practice=false,scenario=null}){
  ...
  this.seed=seed>>>0||1;this.s={version:VERSION,id,mode,practice,startAt:now,endAt:now+duration,regulationEnd:now+duration,phase:'day',ended:false,now,revision:0,events:[],seq:0,control:[0,0],controlDuration:0,rally:[null,null],holds:[{},{}],score:[0,0],winner:null,
 ...
 hit(who,amount,now,{ignoreShield=false}={}){
  ...
  if(!foes.length){dealt=Math.round(damage*1.2);t.dmg[who.side]+=dealt;who.damage+=dealt;}
  else{for(const h of foes){if(damage<=0)break;h.shieldSlots.sort((a,b)=>a-b);while(!ignoreShield&&damage>0&&h.shieldSlots.length){const v=Math.min(damage,h.shieldSlots[0]);h.shieldSlots[0]-=v;damage-=v;absorbed+=v;h.absorbed+=v;if(h.shieldSlots[0]<=0){h.shieldSlots.shift();who.shieldsBroken++;}}
    if(damage<=0)break;const v=Math.min(damage,h.hp);h.hp-=v;damage-=v;dealt+=v;who.damage+=v;t.dmg[who.side]+=v;
    if(h.hp<=0){h.downUntil=now+45000;h.shieldSlots=[];who.kos++;this.event('ko',`${who.name} defeated ${h.name}`,now,{actor:who.id,target:h.id,tower:ti});}
   }
   if(foes.every(h=>h.hp<=0)){const breach=Math.round(foes.reduce((a,h)=>a+h.maxHp,0)*.15);t.dmg[who.side]+=breach;who.damage+=breach;dealt+=breach;}
  }
  const after=this.leader(t);if(after!==before&&after===who.side){if(before!==-1)who.flips++;t.prev=after;who.focus=Math.min(8,who.focus+1);who.spell=Math.min(2,who.spell+1);this.event('capture',`Tower ${t.name} captured`,now,{actor:who.id,side:who.side,tower:ti});}
 ...
 act(id,input,now){const h=this.hero(id);check(!this.s.ended,'Match has ended');check(h.hp>0,'Wait to respawn');check(input&&typeof input.type==='string','Invalid action');let result={type:input.type};
  if(input.type==='move'){check(Number.isInteger(input.tower)&&input.tower>=0&&input.tower<10,'Invalid tower');check(now>=h.moveAt,'Moving too fast');h.tower=input.tower;h.moveAt=now+700;result.tower=h.tower;}
   ...
   check(now>=h.rollAt,'Dice are still settling');const mult=input.allIn?Math.max(4,h.focus*2):(input.mult||1),req={1:0,2:2,3:4,4:6};check(input.allIn?h.focus>=3:(Number.isInteger(mult)&&req[mult]!==undefined&&h.focus>=req[mult]),'Multiplier unavailable');const cost=h.rampage>0?0:input.allIn?h.focus:mult;check(h.focus>=cost,'Not enough Focus');
   h.focus-=cost;h.focusSpent+=cost;h.rollAt=now+2200;h.rolls++;if(cost>0&&!h.bot&&!h.substitute){h.paidRolls++;h.xpEarned+=cost;}
   const f=this.faces();if(h.hot){f[2]=f[0];h.hot=false;h.streak=0;}if(h.forcedCrits>0){f[2]='C';if(f[0]!=='C'&&f[1]!=='C')f[1]='C';h.forcedCrits--;}
   const r=resolve(f);h.lastFaces=f;const triple=r.tier==='triple';if(triple){h.triples++;h.spell=Math.min(2,h.spell+(this.s.endAt-now<=30000?2:1));}
   ...
   if(rallyHere&&h.id!==rally.leader&&!rally.rollers.includes(h.id)){rally.rollers.push(h.id);this.hero(rally.leader).rallyAssists++;}
   let strike={dealt:0,absorbed:0};
   if(r.action==='S'||r.action==='C'){strike=this.hit(h,r.units*h.atk*mult*surge*rm*this.power(now)*(h.streak>=5?1.1:1)*(h.rampage>0?2:1)*(r.action==='C'?3+CHAR[h.char].crit:1)*(r.action==='C'&&triple?1.25:1),now);}
   ...
   else if(r.action==='F'){h.giftDamage+=Math.round(500*mult*(triple?3:1));h.gifts++;}
   else if(r.action==='G')h.goldEarned+=Math.round(({none:5,pair:25,triple:200}[r.tier])*mult*surge*(CHAR[h.char].gold||1));
   ...
   h.spell--;h.spellsCast++;h.spellAt=now+650;
   if(input.spell==='barrage')Object.assign(result,this.hit(h,2*h.atk*1.5*this.power(now),now));
  ...
  else if(input.type==='gift'){check(h.giftDamage>0,'No stored attack to send');const target=this.hero(input.target);check(target.id!==h.id&&target.side===h.side,'Choose a teammate');target.storedDamage=(target.storedDamage||0)+h.giftDamage;h.giftDamage=0;this.event('gift',`${h.name} sent an attack to ${target.name}`,now,{actor:id,target:target.id});}
   ...
   check(h.ult>=100,'Ultimate is not ready');h.ult=0;h.ultsUsed++;
   ...
   if(h.char===4)for(let i=0;i<6;i++)this.hit(h,h.atk*2*this.power(now),now);
 ...
 tick(now){const s=this.s;if(s.ended)return;const prev=s.now,dt=cap(now-prev,0,5000);s.now=now;
  ...
  for(const h of s.heroes){if(h.hp<=0&&now>=h.downUntil){h.hp=h.maxHp;h.downUntil=0;this.event('respawn',`${h.name} returned`,now,{actor:h.id});}
   ...
   if(h.hp>0&&this.leader(s.towers[h.tower])===h.side&&s.heroes.some(a=>a.side!==h.side&&a.tower===h.tower&&a.hp>0))h.defenseSeconds+=dt/1000;
  }
  const o=s.objective;
  ...
  for(const h of s.heroes){if(!(h.bot||h.substitute)||h.hp<=0||now<h.nextBot)continue;h.nextBot=now+2600+Math.floor(this.random()*2700);
   try{if(o.status==='active'&&this.random()<.4)h.tower=o.tower;else if(this.random()<.12)h.tower=Math.floor(this.random()*10);
    ...
    if(h.giftDamage){const friend=s.heroes.find(a=>a.id!==h.id&&a.side===h.side);if(friend)this.act(h.id,{type:'gift',target:friend.id},now);}if(h.storedDamage)this.act(h.id,{type:'stored'},now);
 ...
 result(id){const h=this.hero(id),s=this.s;check(s.ended,'Match is not complete');const eligible=!s.practice&&h.paidRolls>=5,win=s.winner===h.side,allies=s.heroes.filter(a=>a.side===h.side),champ=h.damage>0&&allies.every(a=>a.damage<=h.damage),breaker=h.shieldsBroken>0&&allies.every(a=>a.shieldsBroken<=h.shieldsBroken),
...
return {VERSION,CAPACITY,SEARCH_MS,MATCH_MS,OVERTIME_MS,SPELLS,MODES,Engine,resolve,loadout,weekKey,dailyMode};
});
