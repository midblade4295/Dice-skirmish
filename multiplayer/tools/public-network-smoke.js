...
if(!base||!output)throw Error('Usage: node public-network-smoke.js <approved arena base URL> <report.json>');
...
async function main(){
 const health=(await raw(base+'/health')).data;must(health.apiBuild===113&&health.transportVersion===2&&health.queueMs===20000&&health.capacity===20,'Wrong deployed API');
 record('Public endpoint advertises transport2, 20-second search, 20 slots and unchanged balance110',health);
 for(let i=0;i<2;i++){const r=await raw(base+'/session',{name:'Sync verification '+(i+1)});must(r.status===201,'Session creation failed');clients.push({...r.data,decoder:new W.Decoder(),lastAction:0,downSeen:false});}
 ...
 while(Date.now()-battleStarted<380000){
  ...
  for(let i=0;i<clients.length;i++){
   ...
   if(command){await request(c,'/action',{...command,matchId:room,actionId:crypto.randomUUID()});c.lastAction=snap.now;}
  }
 ...
 must(disconnected&&reconnected,'Same-slot disconnect/reconnect was not observed');record('Heartbeat loss substituted a labelled bot, then restored the same slot',{disconnected,reconnected});
